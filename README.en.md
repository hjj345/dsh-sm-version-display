...
- Fixed the DSH 0.1.7 sidebar version card icon crash: version card and toast icons now use the `*Regular` components exported by the new `dsh-client-ui-primitives`; the legacy `*Outline16`/`*Outline14` icons are no longer exported, and referencing them as undefined triggered React #130, crashing the whole `sidebar.footer.action` slot.
- Tooltip labels now use plain text strings to match the new component signature.
- `handleRefresh` now returns the check promise so tests can await a refresh.
- Tests now cover real VersionCard rendering (wide card, tooltip, refresh button state, and the absence of legacy icon names), with fixed button node matching and hook state reset.
- Updated the package, client About page, release contract, and bilingual documentation to plugin version `v1.2.17` with release date `2026-09-26`.
...
- Fixed Remote Settings response unwrapping, explicitly injected `remote.settings`, and refreshed external settings changes.
