import { existsSync, mkdirSync, mkdtempSync, readFileSync, renameSync, rmSync, writeFileSync, symlinkSync, readlinkSync } from "node:fs";
...
if (args.includes("--self-test")) {
	...
	try {
		...
		writeFileSync(join(profileRoot, "package.json"), JSON.stringify({ name: "dsh-profile-web", private: true }));
...
function fail(reason, error) {
	...
	const actions = isBackupComplete(state.backup, state.id) ? ["repair", "rollback"] : state.backup?.status === "skipped" ? ["repair"] : ["retry-backup"];
...
async function verifyOnly() {
	try {
		...
		save({ status: "success", stage: restartRequired ? "restart-required" : "verification-complete", action: "verify", actions: [], restartRequired, finishedAt: Date.now() });
	} catch (error) {
