...
function spawnCommand(command) {
	const text = [command.executable, ...command.args.map(formatCommandArg)].join(" ");
	if (process.platform !== "win32") return { ...command, text };
	return { executable: process.env.ComSpec ?? "cmd.exe", args: ["/d", "/s", "/c", text], text };
}
...
function startUpdate(command, source, version, method) {
	...
	const run = (index) => {
		const current = spawnCommand(command.commands[index]);
		child = spawn(current.executable, current.args, { stdio: ["ignore", "pipe", "pipe"], windowsHide: true, shell: false });
		...
		child.once("close", (code) => {
			if (code !== 0) finish({ status: "error", reason: "command-failed", exitCode: code });
			else if (index + 1 < command.commands.length) run(index + 1);
			else finish({ status: "success" });
		});
	};
	// ponytail: repair the global tree once when pnpm's global layout is stale; pnpm has no compatible per-command override.
	run(0);
	return job;
}
