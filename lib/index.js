...
function resolveVirtualStoreDir() {
	...
	for (let depth = 0; current !== undefined && depth < 10; depth++, current = dirname(current)) {
		try {
			...
			const match = metadata.match(/^\s*"?virtualStoreDir"?\s*:\s*(.+?)\s*$/m);
...
function updateCommand(method, version) {
	...
	const virtualStoreDir = method === "pnpm" ? resolveVirtualStoreDir() : undefined;
	const args = method === "npm"
		? ["install", "--global", packageSpec]
		: method === "pnpm"
			? ["add", "--global", ...(virtualStoreDir === undefined ? [] : ["--virtual-store-dir", virtualStoreDir]), packageSpec]
			: undefined;
	const command = args === undefined ? undefined : { executable: method === "npm" ? "npm" : "pnpm", args };
