...
async function fetchGithubLatest() {
	...
	const npmAvailable = await isNpmVersionAvailable(item.version);
	return {
		version: item.version,
		type: item.type,
		prerelease: item.release.prerelease === true,
		publishedAt: item.release.published_at,
		tagName: item.release.tag_name,
		url: "https://github.com/deepseek-ai/deepseek-harness/releases/tag/" + encodeURIComponent(item.release.tag_name),
		npmAvailable
	};
}
...
async function fetchVersionCheck() {
	...
	return {
		...
		github: github.status === "fulfilled" ? { status: "success", ...github.value } : { status: "error" },
		checkedAt: Date.now()
	};
}
