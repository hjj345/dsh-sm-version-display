...
function registerRoutes(ctx) {
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			if (body?.jobId !== state.id || !["repair", "rollback"].includes(body?.action)) {
				writeJson(res, 400, { ok: false, reason: "invalid-action" });
				return;
			}
			const action = body.action;
			const next = { ...state, status: "running", stage: action === "repair" ? "repairing" : "rolling-back", action, actions: [], error: null, lines: [...(state.lines ?? []), "[action] " + action] };
