...
function registerRoutes(ctx) {
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			if (mutationPending || state === undefined || (state.status !== "error" && !heartbeatExpired)) return writeJson(res, 409, { ok: false, reason: 'no-failed-update', job: serializeUpdateJob() });
			...
			if (!failureActions(state).includes(action)) return writeJson(res, 409, { ok: false, reason: 'invalid-action' });
