...
`;
		...
		async function requestUpdateAction(action) {
			const job = updateStore.state.job;
			if (job?.id === undefined || job.status !== "error") return;
			try {
