...
`;
		...
		function BackupManager({ t, job }) {
			...
			const refresh = async (directory, keepMessage = false) => { setBusy(true); if (!keepMessage) setMessage(""); try { const next = await backupRequest(directory ? "/scan" : "", directory ? { directory } : undefined); setCatalog(next); setSelected((current) => current.filter((id) => next.backups.some((entry) => entry.id === id))); } catch (err) { setMessage(String(err.message)); } finally { setBusy(false); } };
