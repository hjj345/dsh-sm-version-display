...
`;
		...
		function UpdateOutputPanel({ state, t, onAction, onCopy, copiedCommand }) {
			...
			const backupProgress = job.backup?.phase === "scanning" ? h("progress", { style: { width: "100%", marginTop: "8px" } }) : job.backup?.phase === "copying" ? h("progress", { max: 100, value: job.backup.progress ?? 0, style: { width: "100%", marginTop: "8px" } }) : null;
			...
			const currentCommand = activeStep?.command ?? t("settings.noCommand");
			const actions = (job.status === "error" || heartbeatExpired) && Array.isArray(job.actions) ? h("div", { className: "dvd-settings-update-actions" }, job.actions.includes("wait") ? h("button", { type: "button", className: "dvd-settings-update-action dvd-settings-update-action--wait", onClick: () => onAction("wait") }, t("settings.continueWaiting")) : null, job.actions.includes("repair") ? h("button", { type: "button", className: "dvd-settings-update-action dvd-settings-update-action--repair", onClick: () => onAction("repair") }, t("settings.repair")) : null, job.actions.includes("rollback") ? h("button", { type: "button", className: "dvd-settings-update-action dvd-settings-update-action--rollback", onClick: () => onAction("rollback") }, t("settings.rollback")) : null) : null;
			...
			const result = heartbeatExpired ? h("p", { className: "dvd-settings-source-hint" }, t("settings.heartbeatHelp")) : job.status === "success" ? h("p", { className: "dvd-settings-source-hint" }, job.stage === "rollback-complete" ? t("settings.rollbackFinished") : job.action === "repair" ? t("settings.repairFinished") : t("settings.updateSuccess"), " ", t("settings.restart")) : job.status === "error" ? h(Fragment, null, h("p", { className: "dvd-settings-source-hint" }, t("settings.updateFailed")), h("p", { className: "dvd-settings-source-hint" }, t("settings.manualRepair"))) : null;
			return h("section", { className: "dvd-settings-update-log", "aria-live": "polite" }, h("h3", null, t("settings.updateBoard")), h("p", { className: "dvd-settings-source-hint" }, t("settings.targetVersion"), ": ", formatVersion(job.version)), stepList, backupInfo, backupProgress, peerWarning, h("p", { className: "dvd-settings-update-command" }, currentCommand), h("pre", { className: "dvd-settings-update-output" }, job.lines?.slice(-10).join("\n") || t("settings.waitingOutput")), manual, actions, result);
		}
		...
		function VersionSettingsPage({ scope }) {
			...
			return h(Fragment, null, h("div", { className: "dvd-settings-page" }, hero, error !== null ? h("p", { className: "dvd-settings-error", role: "alert" }, t(error)) : null, general, about, install, h(UpdateOutputPanel, { state: updateState, t, onAction: requestUpdateAction, onCopy: copyCommand, copiedCommand })), modal, confirmModal);
		}
