...
`;
		...
		function UpdateOutputPanel({ state, t, onAction }) {
			...
			const actions = (job.status === "error" || heartbeatExpired) && Array.isArray(job.actions) ? h("div", { className: "dvd-settings-source-actions" }, job.actions.includes("wait") ? h("button", { type: "button", className: "dvd-settings-secondary", onClick: () => onAction("wait") }, t("settings.continueWaiting")) : null, job.actions.includes("repair") ? h("button", { type: "button", className: "dvd-settings-primary", onClick: () => onAction("repair") }, t("settings.repair")) : null, job.actions.includes("rollback") ? h("button", { type: "button", className: "dvd-settings-secondary", onClick: () => onAction("rollback") }, t("settings.rollback")) : null) : null;
		...
		function VersionSettingsPage({ scope }) {
			...
			return h(Fragment, null, h("div", { className: "dvd-settings-page" }, hero, error !== null ? h("p", { className: "dvd-settings-error", role: "alert" }, t(error)) : null, general, about, install, h(UpdateOutputPanel, { state: updateState, t, onAction: requestUpdateAction })), modal, confirmModal);
		}
