...
`;
		...
		function BackupDialog({ title, onClose, children }) {
			...
			return h("dialog", { ref: dialog, className: "dvd-settings-modal", "aria-label": title, onCancel: (event) => { event.preventDefault(); onClose(); }, style: { maxWidth: "min(680px, 90vw)", maxHeight: "85vh", overflow: "auto", color: "inherit", background: "var(--dsw-alias-bg-base, white)" } }, h("h2", null, title), children);
		}
		function DirectoryPicker({ initial, t, onSelect, onClose }) {
			...
			return h(BackupDialog, { title: t("settings.backupDirectory"), onClose }, h("form", { onSubmit: (event) => { event.preventDefault(); void open(path); } }, h("input", { "aria-label": t("settings.backupDirectory"), value: path, onChange: (event) => setPath(event.target.value), style: { width: "100%" } }), h("button", { type: "submit", disabled: busy }, t("settings.openDirectory"))), error ? h("p", { role: "alert" }, error) : null, h("button", { type: "button", disabled: busy || !listing?.parent, onClick: () => open(listing.parent) }, t("settings.parentDirectory")), h("ul", { style: { maxHeight: "35vh", overflow: "auto", overflowWrap: "anywhere" } }, (listing?.directories ?? []).map((entry) => h("li", { key: entry.path }, h("button", { type: "button", disabled: busy, onClick: () => open(entry.path) }, entry.name)))), h("div", { className: "dvd-settings-modal-actions" }, h("button", { type: "button", onClick: onClose }, t("settings.cancel")), h("button", { type: "button", disabled: busy || !listing || path !== listing.path, onClick: () => onSelect(listing.path) }, t("settings.useDirectory"))));
		}
		function BackupManager({ t, job }) {
			...
			const refresh = async (directory, keepMessage = false) => { setBusy(true); if (!keepMessage) setMessage(""); try { setCatalog(await backupRequest(directory ? "/scan" : "", directory ? { directory } : undefined)); setSelected([]); } catch (err) { setMessage(String(err.message)); } finally { setBusy(false); } };
