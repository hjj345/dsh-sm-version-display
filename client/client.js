...
`;
		...
		Object.assign(zh, { "settings.channel.latest": "稳定渠道 latest" });
		Object.assign(en, { "settings.channel.latest": "Stable channel (latest)" });
		Object.assign(zhTW, { "settings.channel.latest": "穩定渠道 latest" });
		...
		function VersionCard({ wide, t, scope }) {
			...
			const handleRefresh = react.useCallback(() => {
				...
				requestCheck(true).then((result) => {
					...
					else if ([result.npm, result.github].some((item) => updateComparison(result.current, item) === 1)) { const item = [result.npm, result.github].find((candidate) => updateComparison(result.current, candidate) === 1); setToast({ kind: "update", seq: Date.now(), text: t("toast.update", { latest: formatVersion(item.version), current: formatVersion(result.current) }) }); }
		...
		function VersionSourceCard({ source, current, item, installInfo, installMethod, t, onUpdate, onCommands }) {
			...
			if (item?.status !== "success") return h("section", { className: "dvd-settings-source-card", "data-kind": "error" }, h("div", { className: "dvd-settings-source-title" }, label, h("span", { className: "dvd-settings-source-badge" }, t("settings.checkError"))), h("p", { className: "dvd-settings-source-hint" }, t("settings.checkError")));
			const canUpdate = installInfo.canOneClick && (source !== "github" || item.npmAvailable === true);
			return h("section", { className: "dvd-settings-source-card", "data-kind": meta.kind }, h("div", { className: "dvd-settings-source-title" }, h("span", null, label), h("span", { className: "dvd-settings-source-badge" }, meta.title)), h("dl", { className: "dvd-settings-source-facts" }, h("dt", null, t("settings.currentVersion")), h("dd", null, formatVersion(current)), h("dt", null, t("settings.latestVersion")), h("dd", null, formatVersion(item.version)), h("dt", null, t("settings.versionType")), h("dd", null, releaseTypeLabel(item.type, t)), source === "github" ? h(Fragment, null, h("dt", null, t("settings.releaseDate")), h("dd", null, formatDate(item.publishedAt)), h("dt", null, t("settings.releaseTag")), h("dd", null, item.tagName), h("dt", null, t("settings.releaseLink")), h("dd", null, h("a", { href: item.url, target: "_blank", rel: "noreferrer" }, item.url))) : h("dt", null, t("settings.channel")), source === "npm" ? h("dd", null, t("settings.channel." + item.channel)) : null, h("dt", null, t("settings.installMethod")), h("dd", null, installMethod), h("dt", null, t("settings.profilePackageManager")), h("dd", null, installMethodLabel(installInfo.profilePackageManager, t))), meta.kind === "update" ? h(Fragment, null, h("div", { className: "dvd-settings-source-actions" }, h("button", { type: "button", className: "dvd-settings-primary", onClick: () => onUpdate(source, item), disabled: !canUpdate }, t("settings.updateNow")), h("button", { type: "button", className: "dvd-settings-secondary", onClick: () => onCommands(source, item) }, t("settings.updateCommand"))), !canUpdate ? h("p", { className: "dvd-settings-source-hint" }, source === "github" && item.npmAvailable !== true ? t("settings.githubManualOnly") : t("settings.updateUnavailable")) : null) : null);
		}
