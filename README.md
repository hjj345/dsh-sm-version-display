...
- 重构侧栏版本入口：底部槽位改为紧凑状态胶囊（状态点 + 版本号）与独立刷新圆钮，内容短小天然自适应任何侧栏宽度，彻底解决窄侧栏下版本卡片溢出、刷新按钮被裁的问题。
- 点击胶囊或折叠栏图标弹出 body portal 全屏面板，展示版本、检测状态、npm/GitHub 结果与刷新按钮，宽度与侧栏布局链解耦。
- 弹层与座位样式全量使用 DSH 主题变量并补充暗色覆盖，明亮/暗色模式均正确渲染；支持 Esc、遮罩点击、关闭按钮三种关闭方式及 prefers-reduced-motion。
- 三处 Tooltip 方位改为 `side:"top"`：气泡显示在锚点上方并按视口 12px 边距钳制、放不下自动翻转，修复胶囊位于侧栏底部时悬浮提示越出视口底沿的问题。
- 测试同步覆盖座位结构、弹层开关、宽/窄两分支行为与 Tooltip 方位断言。
- 更新插件包、客户端关于页面、发布契约和双语文档的版本标识为 `v1.2.19`，发布日期为 `2026-09-26`。
...
- 修复 DSH 0.1.7 侧栏版本卡片图标崩溃：版本卡片与 toast 图标改用新版 `dsh-client-ui-primitives` 导出的 `*Regular` 组件；旧 `*Outline16`/`*Outline14` 图标已不再导出，引用 undefined 会触发 React #130，导致整个 `sidebar.footer.action` 插槽渲染崩溃。
- Tooltip 提示按新版组件签名改为纯文本字符串。
- `handleRefresh` 返回检测 promise，便于测试等待刷新完成。
- 测试补齐 VersionCard 真实渲染断言（宽卡片、tooltip、刷新按钮状态、旧图标名绝迹），并修正按钮节点匹配与 hook 状态复位。
- 更新插件包、客户端关于页面、发布契约和双语文档的版本标识为 `v1.2.17`，发布日期为 `2026-09-26`。
